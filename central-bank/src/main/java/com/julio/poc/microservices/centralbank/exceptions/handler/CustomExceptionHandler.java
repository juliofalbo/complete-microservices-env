package com.julio.poc.microservices.centralbank.exceptions.handler;
...
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    private ResponseEntity<APIExceptionResponse> feignExp(Exception ex, WebRequest request, HttpStatus status) {
        APIExceptionResponse APIExceptionResponse = new APIExceptionResponse(new Date(), ex.getMessage(), request.getDescription(false));
        return ResponseEntity.status(status).body(APIExceptionResponse);
    }
